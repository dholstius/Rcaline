...
###       command line
### Time-stamp: <2009-01-21 23:33:00 ggorjan>
### $Id: Sweave.sh 342 2009-04-01 21:32:10Z ggorjan $
###------------------------------------------------------------------------
...
###
### o remove also figures in folder defined via prefix.string - think about adding 
###   additional option for this, because one probably creates additional folder
###   for figures in order to save them ...
###
### o Consider adding the HTML support
...
###   TEXMFHOME = *
###
### - add a new variable something like (where your Windows TeX installation is)
###   TEXMFWIN = /cygdrive/c/Programs/tex/texmf
###
### - find the lines where TEXMFLOCAL and TEXMFHOME appear and put TEXMFWIN
...
###   TEXMF = {!!$TEXMFCONFIG,!!$TEXMFVAR,$TEXMFHOME,!!$TEXMFLOCAL,!!$TEXMFSYSVAR,!!$TEXMFMAIN,!!$TEXMFWIN}
###   SYSTEXMF = $TEXMFLOCAL;$TEXMFMAIN;$TEXMFDIST;$TEXMFWIN
###
...
# Internal defaults for viewers - used if nothing else is given
PDFAPPIN=acroread
PSAPPIN=gv
DVIAPPIN=xdvi
...
DESCRIPTION
    ...
    -c=, --cache=
      ...
      to set the caching directory via setCacheDir() function in R. The default
      is to use the current directory. Note that this option can be overriden by
      a call to setCacheDir() within the Sweave file! See also --weaver option.
    ...
    -sw, --skipweaver
      Do not use cached information as used by the weaver R package. Implies
      --weaver option.
    ...
    -w, --weaver
      Use caching as implemented in the weaver R package. You need to 
      install the weaver packge in R before this option can be used. See
      http://www.bioconductor.org/packages/bioc/html/weaver.html for the details
      of the caching process. Implies --cache option. See also --skipweaver option.
    ...
    -old=acroread,  --openlatex2pdf=acroread
      ...
      defined, the defaults are '${PDFAPPIN}' for PDF and '${PSAPPIN}' for 
      Postscript output. Variables PDFAPP and PSAPP need to be exported - see
      examples.
...
EXAMPLE
    ...
    Cache intermediate calculations in the .cache directory
...
for OPTION in "$@"; do
    case "$OPTION" in
        -c* | --cache*)
            ...
            OPTIONS_ECHO="$OPTIONS_ECHO - caching as implemented in the cacheSweave R package\n"
        ...
        -sw | --skipweaver )
            if [ "WEAVER" = "no" ]; then
                CACHE=yes
                WEAVER=yes
                OPTIONS=$(echo $OPTIONS | sed -e "s/--weaver//"\
                                              -e "s/-w//")
                OPTIONS_ECHO="$OPTIONS_ECHO - caching as implemented in the weaver R package\n"
            fi
            SKIPWEAVER=yes
            OPTIONS=$(echo $OPTIONS | sed -e "s|$OPTION||")
            OPTIONS_ECHO="$OPTIONS_ECHO - skippping cache information\n"
            ;;
        -t | --tangle )
            TANGLE=yes
            OPTIONS=$(echo $OPTIONS | sed -e "s|$OPTION||")
...
sweaveWeave ()
{
    ...
    # $5 - cachedir [.|*]
    # $6 - logical [yes|no], use weaver
    # $7 - logical [yes|no], do not use weaver cache information
    COMM=''
    # Get weave info
    if [ "$2" = "yes" ]; then # do weave
        echo "cache $4"
        if [ "$4" = "no" ]; then # cache no
            COMM="${COMM} Sweave(file='$1');"     
        else                     # cache yes
            if [ "$6" = "no" ]; then # package cacheSweave
                COMM="${COMM} library(package='cacheSweave');"
                if [ -n "$5" -a ! "$5" = '.' ]; then # set cachedir
                    if [ ! -d "$5" ]; then
                        echo -e "\nNo such directory: $5"
                        exit 1
                    fi
                ...
                COMM="${COMM} library(package='weaver');"
                if [ "$7" = "yes" ]; then
                    COMM="${COMM} Sweave(file='$1', driver=weaver(), use.cache=FALSE);"
                else
                    COMM="${COMM} Sweave(file='$1', driver=weaver());"
                fi
            fi
        fi
    fi
...
for FILE in $FILES; do
    ...
    if [ ! -n "$RNW" -a ! -n "$SNW" -a ! -n "$NW" -a ! -n "$TEXFILE" ]; then
        ...
        echo "File extension should be one of: .lyx, .Rnw, .Snw., .nw or .tex"
        exit 1
    fi
...
for FILE in $FILES; do
    if [ "$WEAVE" = "yes" -o "$TANGLE" = "yes" ]; then
        sweaveWeave $FILE $WEAVE $TANGLE $CACHE $CACHEDIR $WEAVER $SKIPWEAVER
        sweaveError "R"
    fi
